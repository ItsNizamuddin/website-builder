...
import { ChevronDown, ChevronRight, GraduationCap, Quote, Play, Mail, Phone, MapPin, Menu, X, Headset, MessageSquare, Info, ArrowRight, ExternalLink } from "lucide-react";
...
const resolveThemeColor = (colorVal: string | undefined, defaultHex: string, cssVar: string) => {
...
// 1. Heading Component - Revised for dark text default in Light Mode
export const Heading: React.FC<ComponentProps> = ({ props }) => {
  ...
  return (
    <Tag
      className={`${fontSize} font-bold tracking-tight ${alignClass} ${marginTop} ${marginBottom} transition-colors duration-150`}
      style={{ color }}
    >
      {text}
    </Tag>
...
// 2. Text Component - Revised for slate text default in Light Mode
...
// 3. Image Component - Revised for slate border
export const ImageComp: React.FC<ComponentProps> = ({ props }) => {
  const { src = "", alt = "Image", borderRadius = "rounded-lg", aspectRatio = "aspect-video", marginTop = "mt-4", marginBottom = "mb-4" } = props;

  return (
    <div className={`relative overflow-hidden w-full ${aspectRatio} ${borderRadius} ${marginTop} ${marginBottom} border border-slate-200 group shadow-sm`}>
...
// 4. Button Component - Revised for blue accent theme
export const ButtonComp: React.FC<ComponentProps> = ({ props, mode }) => {
  const { text = "Button", url = "#", variant = "primary", textColor = "#ffffff", size = "md", borderRadius = "rounded-md", align = "left", marginTop = "mt-4", marginBottom = "mb-4" } = props;
  ...
  }[size as "sm" | "md" | "lg"] || "px-5 py-2.5 text-base";
  ...
  return (
    <div className={`${alignClass} ${marginTop} ${marginBottom}`}>
      <a
        href={url}
        onClick={handleClick}
        className={`${baseStyle} ${sizeClass} ${borderRadius} ${variantClass}`}
        style={buttonStyle}
      >
        {text}
      </a>
...
// 5. Video Component - Revised for slate border
...
// 6. Hero Component - Revised for vibrant blue-indigo gradients
export const HeroComp: React.FC<ComponentProps> = ({ props, mode }) => {
  ...
  return (
    <div
      className={`w-full rounded-2xl p-8 md:p-12 ${bgClass} border border-slate-200/20 shadow-xl flex flex-col justify-center ${alignClass} ${marginTop} ${marginBottom}`}
      style={containerStyle}
    >
      <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight max-w-2xl leading-tight text-left" style={{ color: textColor }}>
        {heading}
      </h1>
      <p className="mt-4 text-lg md:text-xl max-w-xl font-light leading-relaxed opacity-90 text-left" style={{ color: textColor }}>
        {subheading}
      </p>
      {buttonText && (
        <a
          ...
          className="mt-8 px-6 py-3 font-bold rounded-lg hover:brightness-105 hover:scale-[1.02] active:scale-[0.98] smooth-transition shadow-lg shadow-black/10"
...
// 7. Testimonial Component - Revised for light glass theme
export const TestimonialComp: React.FC<ComponentProps> = ({ props }) => {
  ...
  return (
    <div
      className={`p-6 rounded-xl flex flex-col justify-between h-full ${marginTop} ${marginBottom} shadow-sm border border-slate-200/70 relative group overflow-hidden`}
    ...
    >
      ...
      <p className="text-base italic leading-relaxed relative z-10 text-left" style={{ color: textColor }}>
        &ldquo;{quote}&rdquo;
      </p>
      <div className="flex items-center gap-4 mt-6 border-t border-slate-100 pt-4 z-10">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
        ...
        <div className="text-left">
          <h4 className="font-semibold text-sm" style={{ color: authorColor }}>{author}</h4>
          <p className="text-xs" style={{ color: textColor, opacity: 0.7 }}>
            {role} &bull; <span className="font-semibold" style={{ color: resolvedAccent }}>{company}</span>
          </p>
        </div>
      </div>
    </div>
...
// 8. FAQ Component - Revised for light white-box tiles with blue indicators
      ...
      {(items || []).map((item: any, idx: number) => {
        ...
        return (
          <div
            ...
            style={{ backgroundColor: bgColor, borderColor: isOpen ? `color-mix(in srgb, ${resolvedAccent} 18%, transparent)` : "#e2e8f0" }}
          >
            <button
              onClick={() => toggle(idx)}
              className="w-full flex items-center justify-between p-4 text-left font-semibold smooth-transition cursor-pointer"
              style={{ color: textColor }}
            >
              <span>{item.question || "New Question?"}</span>
              <ChevronDown
            ...
            >
              <div
                className="p-4 text-sm leading-relaxed text-left"
...
// 9. Course Card Component - Revised to match SkillDeck's style
export const CourseCardComp: React.FC<ComponentProps> = ({ props, mode }) => {
  ...
  return (
    <div
      ...
      className={`rounded-xl overflow-hidden border smooth-transition flex flex-col justify-between h-full shadow-md group ${marginTop} ${marginBottom}`}
      style={{
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center gap-1.5 text-xs mb-2 font-semibold" style={{ color: resolvedAccent }}>
          <GraduationCap className="w-3.5 h-3.5" />
          <span>{duration}</span>
        </div>
        <h3
          className="font-bold text-lg leading-snug smooth-transition text-left"
          style={{ color: isHovered ? resolvedAccent : textColor }}
        >
          {title}
        </h3>
        <p className="text-xs mt-1.5 flex-1 text-left" style={{ color: detailsColor }}>
          By {instructor}
        </p>
...
const MOCK_RESOURCES = [
  ...
  "Cracking The PMP Exam: Exam Preparation Tips From Experts",
  "What Is PMP® Certification — And Is It Worth It In 2025?",
  "PMP Vs CAPM Vs PRINCE2: Which Certification Should You Choose?",
  "Agile Methodology: A Beginner's Guide",
  "Salesforce Admin Secrets Every Beginner Needs",
  "CSM Exam Preparation Blueprint",
  "The Rise of Artificial Intelligence in PM"
];
...
// 10. Navbar Component - SkillDeck Styled Light Theme Navbar
export const NavbarComp: React.FC<ComponentProps> = ({ props, mode }) => {
  ...
  const isFullWidth = props.navbarWidth === "full-width";
  ...
  const navLayoutClasses = isFullWidth
    ...
    : `relative w-full ${contentMaxWidthClass} mx-auto py-4 px-4 sm:px-6 rounded-xl border border-slate-200/80 shadow-sm flex items-center justify-between ${itemsGap} ${bgClass} ${finalMarginTop} ${marginBottom}`;
      ...
      <div className={`flex items-center ${itemsGap} shrink-0`}>
        ...
        {showLogo && (
          <a
            href={logoUrl || "#"}
            onClick={handleLinkClick}
            className="font-extrabold text-lg tracking-tight cursor-pointer flex items-center gap-2 shrink-0 hover:opacity-90 smooth-transition"
            style={{ color: textColor }}
          >
            {logoImgToUse ? (
              // eslint-disable-next-line @next/next/no-img-element
          ...
          {links.map((link: any, idx: number) => {
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-bold hover:opacity-80 smooth-transition cursor-pointer select-none"
                    style={{ color: textColor }}
                  >
            ...
            return (
              <a
                ...
                href={link.url || "#"}
                onClick={handleLinkClick}
                className="text-xs font-bold hover:opacity-80 smooth-transition"
                style={{ color: textColor }}
              >
                {link.label || "Link"}
              </a>
...
// 11. Footer Component - SkillDeck Styled Light Theme Footer
export const FooterComp: React.FC<ComponentProps> = ({ props, mode }) => {
  ...
  const isFullWidth = footerWidth === "full-width";
  const contentMaxWidthClass = {
    "5xl": "max-w-5xl",
    "6xl": "max-w-6xl",
    "7xl": "max-w-7xl",
      ...
      <div className={`${footerGridClass} gap-8 text-left`}>
        ...
        {showBrandColumn !== false && (
          <div className="flex flex-col gap-3">
            <span className="font-extrabold text-xl tracking-tight flex items-center gap-1.5" style={{ color: textColor }}>
              {logoImgToUse ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={logoImgToUse} alt="Logo" className="h-8 w-auto max-w-[150px] object-contain rounded" />
              ) : (
            ...
            <p className="text-xs opacity-70 leading-relaxed max-w-xs" style={{ color: textColor }}>
              {brandDescription}
            </p>
            {/* Social Media Links */}
              ...
              <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                {socialFacebook && (
                  <a
                    ...
                    onClick={handleLinkClick}
                    className="opacity-70 hover:opacity-100 smooth-transition p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/5 flex items-center justify-center"
                    style={{ color: textColor }}
                    title="Facebook"
                  >
            ...
            <div key={idx} className="flex flex-col gap-3">
                  ...
                  {(col.links || []).map((link: any, linkIdx: number) => (
                    <li key={linkIdx}>
                      <a
                        href={link.url || "#"}
                        onClick={handleLinkClick}
                        className="text-xs opacity-75 hover:opacity-100 smooth-transition"
                        style={{ color: textColor }}
                      >
                        {link.label || "Link"}
                      </a>
              ...
              {colType === "contact" && (
                <ul className="flex flex-col gap-2.5">
                  {col.email && (
                    <li className="flex items-center gap-2 text-xs opacity-75 hover:opacity-100 smooth-transition">
                  ...
                  {col.address && (
                    <li className="flex items-start gap-2 text-xs opacity-75">
              ...
              {colType === "text" && (
                <p className="text-xs opacity-75 leading-relaxed whitespace-pre-line" style={{ color: textColor }}>
                  {col.text || ""}
                </p>
              )}
            </div>
...
export const RenderComponent: React.FC<RegistryProps> = ({ node, mode = "preview" }) => {
  const getComp = () => {
    switch (node.type) {
      case "navbar":
      ...
      case "heading":
        return <Heading props={node.props} mode={mode} />;
      case "text":
        return <Text props={node.props} mode={mode} />;
      case "image":
        return <ImageComp props={node.props} mode={mode} />;
      case "button":
        return <ButtonComp props={node.props} mode={mode} />;
      case "video":
        return <VideoComp props={node.props} mode={mode} />;
      case "hero":
        return <HeroComp props={node.props} mode={mode} />;
      case "testimonial":
        return <TestimonialComp props={node.props} mode={mode} />;
      case "faq":
        return <FAQComp props={node.props} mode={mode} />;
      case "courseCard":
        return <CourseCardComp props={node.props} mode={mode} />;
      default:
